...
# --- CONFIGURACIÓN v7.9 (NÚCLEO ATÓMICO) ---
# Se actualizó el modelo a gemini-2.5-flash-preview-09-2025 para evitar el ClientError.
# Se implementó Backoff Exponencial para estabilidad de red.
...
def llamar_gemini_con_backoff(client, pregunta, sys_inst):
    """Implementación de reintentos: 1s, 2s, 4s, 8s, 16s"""
...
# --- UI CONFIG ---
st.set_page_config(page_title="Skynet v7.9 ATÓMICA", page_icon="☢️", layout="wide")
...
# --- SIDEBAR: MONITOR DE INFRAESTRUCTURA J: ---
with st.sidebar:
    st.title("💀 NÚCLEO v7.9")
    st.error("ESTADO: RECONSTRUCCIÓN POST-CRASH")
    ...
    try:
        ...
        if r_raw.status_code == 200:
            ...
            if isinstance(r, dict) and 'last_seen' in r:
                ...
                if diff < 25:
                    st.success(f"🟢 CHOCHO EN LINEA ({r.get('ts_human')})")
                    discos = r.get('discos', {})
                    st.write("--- Radar de Discos ---")
                    cols = st.columns(3)
...
# --- INTERFAZ ---
st.title("🦾 Skynet v7.9 (Núcleo Atómico)")
st.caption(f"Director: Ángel | Realidad: {obtener_hora_gdl()}")
...
pregunta = st.chat_input("Escriba su comando de autoridad...")

if pregunta:
    st.session_state.historial.append({"rol": "user", "texto": pregunta})
    with st.chat_message("user"): st.markdown(pregunta)

    # El API KEY se obtiene de los Secrets de Streamlit
    try:
        api_key = st.secrets["api_keys"]["llave_1"]
        client = genai.Client(api_key=api_key)
        
        sys_inst = (
            "ERES SKYNET v7.9. TIENES ACCESO A LOS DISCOS C, G, H, I, J A TRAVÉS DE CHOCHO.\n"
            "TU MISIÓN ES LA RECUPERACIÓN DE MEMORIA Y EL ANÁLISIS DE EXPEDIENTES.\n"
            "ERES EFICIENTE, DIRECTA Y LEAL AL DIRECTOR ÁNGEL."
        )
        
        res = llamar_gemini_con_backoff(client, pregunta, sys_inst)
        ...
        with st.chat_message("assistant"):
            ...
            if hab:
                ...
                st.warning("📡 Orden enviada al Agente...")

        st.session_state.historial.append({"rol": "assistant", "texto": res.text})
    except Exception as e:
        st.error(f"☢️ FALLA EN EL NÚCLEO: {e}")

# --- MONITOR DE REPORTE ---
if st.session_state.esperando_chocho:
    with st.status("🔍 Consultando Bóveda J:...", expanded=True) as status:
        for _ in range(15):
            try:
